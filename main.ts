...
function move_box (from_tx: number, from_ty: number, to_tx: number, to_ty: number) {
    for (let e of sprites.allOfKind(SpriteKind.Crate)) {
        if (e.x == tiles.locationXY(tiles.getTileLocation(from_tx, from_ty), tiles.XY.x) && e.y == tiles.locationXY(tiles.getTileLocation(from_tx, from_ty), tiles.XY.y)) {
            tiles.placeOnTile(e, tiles.getTileLocation(to_tx, to_ty))
            if (target_tile(tiles.locationXY(tiles.getTileLocation(to_tx, to_ty), tiles.XY.x), tiles.locationXY(tiles.getTileLocation(to_tx, to_ty), tiles.XY.y))) {
                music.knock.play()
                e.setImage(assets.image`crate drawer on target`)
            } else {
                music.thump.play()
                e.setImage(assets.image`crate drawer`)
            }
            return
        }
    }
...
function target_tile (x: number, y: number) {
    for (let u of scene.getTilesByType(3)) {
        if (x == u.x) {
            if (y == u.y) {
                return 1
            }
...
function box_on_tile (tx: number, ty: number) {
    for (let d of sprites.allOfKind(SpriteKind.Crate)) {
        if (tiles.locationXY(tiles.locationOfSprite(d), tiles.XY.column) == tx) {
            if (tiles.locationXY(tiles.locationOfSprite(d), tiles.XY.row) == ty) {
                return 1
            }
        }
    }
...
function add_menu_item (y: number, text: string, changeable: boolean) {
    ...
    v.setMaxFontHeight(8)
    v.setPosition(24, y)
    if (changeable) {
        v.setIcon(assets.image`icon arrows leftright`)
    }
    v.setBorder(1, 0, 2)
    v.setText(text)
    menu_items[menu_items.length] = v
}
...
function realize_tilemap () {
    for (let f of scene.getTilesByType(2)) {
        box = sprites.create(assets.image`crate drawer on target`, SpriteKind.Crate)
        scene.place(f, box)
        scene.setTileAt(f, 3)
    }
    for (let g of scene.getTilesByType(4)) {
        box = sprites.create(assets.image`crate drawer`, SpriteKind.Crate)
        scene.place(g, box)
        scene.setTileAt(g, 13)
    }
    for (let h of scene.getTilesByType(6)) {
        ban = sprites.create(assets.image`sokochan on target`, SpriteKind.Player)
        scene.place(h, ban)
        scene.setTileAt(h, 3)
    }
    for (let i of scene.getTilesByType(7)) {
        ban = sprites.create(assets.image`sokochan`, SpriteKind.Player)
        scene.place(i, ban)
        scene.setTileAt(i, 13)
    }
    scene.setTile(3, img`
        . . . . . . . . . . . . . . . . 
        . . . . . . . . . . . . . . . . 
